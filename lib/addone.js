...
const use = require('../commander/use');
...
module.exports = class Addtion {
  ...
  async asyncWebview(cwd, ...names) {
    let filePath = names.join('/');
    if (!/\.vue$/.test(filePath)) filePath += '.vue';
  ...
  async asyncComponent(cwd, ...names) {
    let filePath = names.join('/');
    if (!/\.vue$/.test(filePath)) filePath += '.vue';
    await this.render('component.ejs', path.resolve(cwd, 'app', 'async-components', filePath), {
      className: this.prefix(...names)
    });
  }

  async component(cwd, ...names) {
    let filePath = names.join('/');
    if (!/\.vue$/.test(filePath)) filePath += '.vue';
    await this.render('component.ejs', path.resolve(cwd, 'app', 'components', filePath), {
      className: this.prefix(...names)
    });
  }

  async directive(cwd, ...names) {
    let filePath = names.join('/');
    if (!/\.js$/.test(filePath)) filePath += '.js';
    await this.render('directive.ejs', path.resolve(cwd, 'app', 'directives', filePath));
  }

  async filter(cwd, ...names) {
    let filePath = names.join('/');
    if (!/\.js$/.test(filePath)) filePath += '.js';
    await this.render('filter.ejs', path.resolve(cwd, 'app', 'filters', filePath));
  }
  
  install(cwd, ...plugins) {
    const npm = use.config().use;
