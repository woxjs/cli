...
module.exports = async (ctx, project, options = {}) => {
  ...
  if (options.controller) {
    switch (options.controller) {
      case 'jsx': fileTemplates.push(templates.jsxController); break;
      default: fileTemplates.push(templates.jsController);
    }
  }
  if (options.service) {
    switch (options.service) {
      case 'jsx': fileTemplates.push(templates.jsxService); break;
      default: fileTemplates.push(templates.jsService);
    }
  }
  ['directive', 'filter', 'mixin', 'middleware', 'decorate', 'webview'].forEach(key => {
